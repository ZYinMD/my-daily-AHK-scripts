...
; ★ note: must use /bin/bash (similar as shebang), can't use /bin/usr/bash, which doesn't work. Not sure why.
  Run,%A_ProgramFiles%\Git\bin\bash.exe %DirDropbox%\Coding\play\features\foobar2000\add-random-songs-to-foobar2000.sh,,Hide ;Hide is an argument which means run silently. Without it the bash shell will pop up then go away.
  ;deprecated:
